...
void TestSortAlgorithm(const std::string &algorithm_name, const std::function<void(std::vector<int> &)> &sort_algorithm,
                       const std::vector<int> &vec, const bool show_vec, const bool reversed) {
    std::vector<int> copied_vec = vec;
    swap_tracker.Reset();
    compare_tracker.Reset();
    if (show_vec) PrintVec(copied_vec);

    Timer timer(algorithm_name, false);
    sort_algorithm(copied_vec);
    timer.Stop();
    ...
    if (IsSorted(copied_vec, reversed)) {
